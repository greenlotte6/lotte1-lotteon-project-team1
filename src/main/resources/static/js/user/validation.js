...
document.addEventListener("DOMContentLoaded", function () {
  ...
  btnCheckId.onclick = function () {
    const value = $("input[name='memberId.user.id']").val();
  ...
  $("input[name='passwordConfirm']").focusout(
      function () {
        const value1 = $("input[name='memberId.user.password']").val();
  ...
  $("input[name='memberId.user.contact']").on('focusout',
      async function () {
  ...
  btnSendEmail.onclick = async function () {
    ...
    const value = $("input[name='memberId.user.email']").val();
