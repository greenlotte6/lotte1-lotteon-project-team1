...
function request(id) {
  fetch(`/admin/coupon/issued/${id}`)
      ...
      .then(couponHistory => {
        ...
        $(".coupon-used-id").text(couponHistory.status);
        $(".coupon-type-id").text(couponHistory.coupon.coupon_type.name); // type_id 대신 name 사용
        $(".coupon-user-id").text(couponHistory.user_id); // type_id 대신 name 사용
